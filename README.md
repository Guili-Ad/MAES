...
基于 MaaFramework 5.12.2 与 ProjectInterface V2 独立实现的《偶像梦幻祭2》打歌自动化项目。
当前版本 **v1.0.0-Stable**，聚焦「临时打歌」与「打歌点位校准」。

## 功能范围

- **打歌点位校准**：在 1280×720 演唱会进行页识别 7/9 个判定点并保存本地校准。
...
1. 从本仓库 Releases 下载 `MAES-v1.0.0-Stable.zip` 并解压到任意目录。
...
3. 打开 `MFAAvalonia.exe`，添加你的模拟器设备；确认设备“输入模式”为 Default 或 Maatouch（打歌必需）。
4. 启动游戏并进入演唱会进行页（画面需为 1280×720），建议保持歌曲暂停窗口可用。
5. 先运行「打歌点位校准」，识别出 7 个判定点后完成。
6. 运行「临时打歌」，整首歌将自动进行；结束时以 LIVE 结算检测为准。
